...
### 🔴 GAP 6: Interpretability Strategy Undefined
**Issue**: Claims to solve "black box" problem but no concrete explainability approach specified.
...
### 🟡 GAP 7: Evaluation Metrics Incomplete
**Issue**: Metrics listed are vague. No specific formulas or fairness considerations.
...
┌─────────────────────────────────────────────────────────────────────────┐
│                    INPUT LAYER (PDF/DOCX)                               │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│ 1. PDF/DOCX Upload 
(PROMPT THE CANDIDATE TO UPLOAD THEIR RESUME IN ONLY THESE FORMATS)     
```
├── PDF:        pdfplumber, PyMuPDF (fitz)
├── DOCX:       python-docx
├── Images/OCR: Tesseract OCR, pytesseract
├── NER:        spaCy with en_core_web_sm or custom NER model
└── Fallback:   textract for generic extraction
```                                            
- Define supported file formats explicitly: `.pdf`, `.docx`
- Implement format detection and routing logic
- Add error handling for corrupted/unsupported files    
│ if candidate uploads unsupported file format, prompt to upload supported file format
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘ 
    │
    ▼
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
**Issue**: How TF-IDF/SVM and BERT work together is unclear. Risk of conflicting predictions.
**Recommendations** - Choose one approach:

| Strategy | Description | When to Use |
|----------|-------------|-------------|
| **Cascade** | TF-IDF/SVM filters → BERT ranks top-N | High volume, need speed |
| **Score Fusion** | Weighted: `0.3*SVM + 0.7*BERT` | Balanced accuracy/speed |
| **Ensemble Voting** | Majority decision from both models | Maximum accuracy |
| **Fallback** | BERT primary, SVM when BERT confidence < threshold | Reliability focus |
```
Integration Flow:
Resume → TF-IDF/SVM (fast filter, <50ms)
       → Top candidates → BERT semantic ranking (100-200ms)
       → Combined score with configurable weights
```                                                                       │
| Model | Parameters | Latency | Best For |
|-------|-----------|---------|----------|
|`sentence-transformers/all-MiniLM-L6-v2` | 22M | ~20ms | Semantic similarity |
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
    │
    ▼
