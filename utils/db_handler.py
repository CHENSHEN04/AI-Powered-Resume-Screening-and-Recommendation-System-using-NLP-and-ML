...
class DatabaseManager:
    ...
    def sync_auth_session(self):
        ...
        NOTE: an earlier version of this method also tried to proactively call
        `auth.refresh_session()` here whenever the token looked close to expiry, to
        work around RLS writes failing on a stale token. That was reverted — Supabase
        refresh tokens are single-use/rotating, and calling refresh_session() outside
        of a carefully-persisted storage layer risks consuming a refresh token without
        durably saving its replacement, which trips Supabase's reuse-detection and can
        invalidate the whole session (a much worse failure than the one it was meant to
        fix). This method now just re-applies whatever access token the client already
        holds, same as before. Expiry is still detected (see `session_is_expired`) so
        callers can show the user a clear "please log out and back in" message instead
        ...
        try:
            ...
            if session:
                ...
                elif isinstance(session, dict) and session.get("access_token"):
                    self.supabase.postgrest.auth(session["access_token"])
        except Exception:
            pass
    ...
    def session_is_expired(self) -> bool:
        """
        Read-only check for whether the current session's access token has expired.
        Does NOT attempt to refresh anything — safe to call anytime, purely informational
        (e.g. to decide whether to tell the user "please log out and back in").
        """
